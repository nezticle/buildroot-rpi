...
define LOCKDEV_INSTALL_STAGING_CMDS
	...
	(cd $(STAGING_DIR)/usr/lib; ln -sf liblockdev.$(LOCKDEV_VERSION).so liblockdev.so)
	(cd $(STAGING_DIR)/usr/lib; ln -sf liblockdev.$(LOCKDEV_VERSION).so liblockdev.so.1)
endef
...
define LOCKDEV_INSTALL_TARGET_CMDS
	...
	(cd $(TARGET_DIR)/usr/lib; ln -sf liblockdev.$(LOCKDEV_VERSION).so liblockdev.so)
	(cd $(TARGET_DIR)/usr/lib; ln -sf liblockdev.$(LOCKDEV_VERSION).so liblockdev.so.1)
endef
