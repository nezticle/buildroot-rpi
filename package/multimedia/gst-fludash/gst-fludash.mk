...
GST_FLUDASH_INSTALL_STAGING = YES

define GST_FLUDASH_INSTALL_TARGET_CMDS
	mkdir -p $(TARGET_DIR)/usr/lib/gstreamer-0.10
	$(INSTALL) -m 755 $(@D)/usr/lib/libfludownloader.so.0.0.0 $(TARGET_DIR)/usr/lib/
	cp $(@D)/usr/lib/libfludownloader.so.0 $(TARGET_DIR)/usr/lib/
	cp $(@D)/usr/lib/libfludownloader.so $(TARGET_DIR)/usr/lib/
